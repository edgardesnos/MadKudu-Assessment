...
def is_valid_url_to_crawl(url):
    ...
    return (
        ...
        and parsed.netloc in ["support.madkudu.com"]
            ...
            try:
                ...
                if driver.current_url not in visited:
                    ...
                    if valid_url_to_store(url):
                        html_contents.append(Document(page_content=driver.page_source, metadata={"source": url}))
                visited.add(url)
                print(f"Elapsed: {time() - start_time:.2f}s, Visited: {url}, Depth: {depth}")
...
Chroma.from_documents(documents=splits, embedding=OpenAIEmbeddings(), persist_directory=db_directory)
